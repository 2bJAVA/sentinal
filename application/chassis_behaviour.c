...
static void chassis_auto_sentinel_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, Typedef_DISTANCE *DIS,chassis_move_t *chassis_move_rc_to_vector);
Typedef_DISTANCE distance;
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
  ...
  //		{
	...
	{
		chassis_behaviour_mode = CHASSIS_AUTO_SENTINEL;
	}
  ...
  // 添加自己的逻辑判断进入新模式
  // 哨兵自动控制逻辑
  if (chassis_behaviour_mode == CHASSIS_AUTO_SENTINEL)
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_AUTO;
  }
  // accord to beheviour mode, choose chassis control mode
  // 根据行为模式选择一个底盘控制模式
  else if (chassis_behaviour_mode == CHASSIS_ZERO_FORCE) // 底盘电机电流控制值为0，底盘表现为无力状态
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_RAW;
  }
  else if (chassis_behaviour_mode == CHASSIS_NO_MOVE) // 底盘电机速度值为0，由于电机有速度环控制，底盘表现为不移动，但推动底盘存在抵抗力
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW;
  }
  else if (chassis_behaviour_mode == CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW) // 底盘移动速度由遥控器和键盘按键一起决定，同时会控制底盘跟随云台，从而计算旋转角度，底盘存在电机速度环和云台角度环控制，是RM常见控制逻辑
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW;
  }
  else if (chassis_behaviour_mode == CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW) // 底盘移动速度和旋转速度均由遥控器决定，不存在速度闭环控制，故而底盘只有速度环控制
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW;
  }
  else if (chassis_behaviour_mode == CHASSIS_NO_FOLLOW_YAW) // 底盘开环控制，不存在程序的闭环控制，遥控器的通道值直接转化为电机电流值发送到CAN总线上
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW;
  }
  else if (chassis_behaviour_mode == CHASSIS_OPEN) // 底盘开环控制，不存在程序的闭环控制，遥控器的通道值直接转化为电机电流值发送到CAN总线上
  {
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_RAW;
  }
}
...
//  }
  ...
  if (chassis_behaviour_mode == CHASSIS_AUTO_SENTINEL)
  {
    chassis_auto_sentinel_control(vx_set, vy_set, angle_set, &distance,chassis_move_rc_to_vector);
  }
  else if (chassis_behaviour_mode == CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW)
  {
    chassis_infantry_follow_gimbal_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
 ...
 if (vx_set == NULL || vy_set == NULL || wz_set == NULL)
  {
	...
	if(DIS->spining_flag == 1 && DIS->x==0 && DIS->y==0)
	{
		//DIS->z=SENTINEL_MIN_ROTATION_SPEED;
		filter_attitude_dispose(); //?a???y?òD?íó?Y
	}
	else if(DIS->spining_flag == 1 &&(DIS->x!=0||DIS->y!=0))
	{
		DIS->z=SENTINEL_MIN_ROTATION_SPEED;
	}
   ...
   if(DIS->spining_flag == 1)
   {
	  spining_arithmetic(&(*vx_set),&(*vy_set),&(*wz_set));
   }
