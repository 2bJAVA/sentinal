...
//云台行为状态机
 gimbal_behaviour_e gimbal_behaviour = GIMBAL_AUTO_SENTINEL;

Typedef_gimbal_control gimbal_distance;

float last_yaw_speed = 0;
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_AUTO_SENTINEL)
		{
			  gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
		}
...
//		{
        ...
        gimbal_behaviour = GIMBAL_ABSOLUTE_ANGLE;
			...
			{
			  gimbal_behaviour = GIMBAL_AUTO_SENTINEL;
			}
...
/**
   * @brief 扫描模式
   * @param[in]  yaw_speed: yaw轴角速度控制，为角度的增量 单位 rad/s
   * @param[in]  gimbal_control_set:云台数据指针
   * @retval none.
   */
void gimbal_scan_control(fp32 *yaw_speed, fp32 *pitch_speed ,gimbal_control_t *gimbal_control_set,Typedef_gimbal_control * GIM)
{
		if (yaw_speed == NULL || gimbal_control_set == NULL)
    {
        return;
    }
	  *yaw_speed =GIM->yaw_speed;//GIM->yaw_speed
		*pitch_speed = GIM->pitch_speed;
}
