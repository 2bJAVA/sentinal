...
typedef enum
{
  ...
  GIMBAL_MOTIONLESS,
  GIMBAL_AUTO_SENTINEL,	
} gimbal_behaviour_e;

typedef struct
{
	first_order_filter_type_t gimbal_cmd_slow_set_yaw_speed;
	first_order_filter_type_t gimbal_cmd_slow_set_pitch_speed;
	float accumulated_yaw;
	float accumulated_pitch;
	float set_start_yaw_angle;
	float set_start_pitch_angle;
	float yaw_speed;
	float pitch_speed;
}Typedef_gimbal_control;

extern Typedef_gimbal_control gimbal_distance;
...
/**
   * @brief 扫描模式
   * @param[in]  yaw_speed,pitch_speed: yaw&pitch轴角速度控制 单位 rad/s
