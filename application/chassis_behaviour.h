...
typedef enum
{
  ...
  CHASSIS_AUTO_SENTINEL // The Sentinel chassis is automatically controlled by PID composed of Angle ring and speed ring
                        // 哨兵底盘自动控制，由角度环和速度环组成的串级PID
} chassis_behaviour_e;
...
typedef struct
{
  ...
  float z; // 转弯修正(速度值)
  float spining_angle;
  double start_yaw_angle;      // 初始yaw角
  double remember_yaw_angle;   // 记录yaw角
  double rotating_yaw_angle;   // 旋转后yaw角
  double correction_yaw_angle; // 修正yaw角
	uint8_t spining_flag;
	uint8_t control_priority;
} Typedef_DISTANCE;

extern Typedef_DISTANCE distance;
